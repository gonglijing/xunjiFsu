...
func (a *PandaXAdapter) Send(data *models.CollectData) error {
	if data == nil {
		slog.Info(fmt.Sprintf("[PandaX-%s] Send: data is nil", a.name))
		return nil
	}

	slog.Info(fmt.Sprintf("[PandaX-%s] Send: deviceId=%d, deviceKey=%s, fields=%d",
	...
	slog.Info(fmt.Sprintf("[PandaX-%s] Send: enqueued, queueLen=%d", a.name, queueLen))
	return nil
}
...
func (a *PandaXAdapter) SendAlarm(alarm *models.AlarmPayload) error {
	...
	slog.Info(fmt.Sprintf("[PandaX-%s] SendAlarm: deviceKey=%s, fieldName=%s, severity=%s, message=%s, queueLen=%d",
		a.name, alarm.DeviceKey, alarm.FieldName, alarm.Severity, alarm.Message, queueLen))

	if queueLen >= a.alarmBatch {
		slog.Info(fmt.Sprintf("[PandaX-%s] SendAlarm: 触发批量上报, batchSize=%d", a.name, a.alarmBatch))
	}
...
func (a *PandaXAdapter) flushRealtime() error {
	...
	slog.Info(fmt.Sprintf("[PandaX-%s] flushRealtime: 开始发送 %d 条数据", a.name, len(batch)))

	topic, body := a.buildBatchRealtimePublish(batch)
	if err := a.publish(topic, body); err != nil {
		slog.Info(fmt.Sprintf("[PandaX-%s] flushRealtime: 发送失败: %v", a.name, err))
		a.dataMu.Lock()
	...
	slog.Info(fmt.Sprintf("[PandaX-%s] flushRealtime: 发送成功 %d 条数据", a.name, len(batch)))
	clear(batch)
	return nil
}
...
func (a *PandaXAdapter) buildBatchRealtimePublish(batch []*models.CollectData) (string, []byte) {
	...
	slog.Info(fmt.Sprintf("[PandaX-%s] buildBatchRealtimePublish: deviceCount=%d, payloadSize=%d", a.name, len(payload), len(body)))
...
func (a *PandaXAdapter) flushAlarmBatch() error {
	...
	slog.Info(fmt.Sprintf("[PandaX-%s] flushAlarmBatch: 开始发送 %d 条报警", a.name, len(batch)))

	successCount := 0
	for _, item := range batch {
		topic, body := a.buildAlarmPublish(item)
		if err := a.publish(topic, body); err != nil {
			slog.Info(fmt.Sprintf("[PandaX-%s] flushAlarmBatch: 发送失败 deviceKey=%s field=%s: %v",
	...
	slog.Info(fmt.Sprintf("[PandaX-%s] flushAlarmBatch: 发送成功 %d/%d", a.name, successCount, len(batch)))
	clear(batch)
	return nil
}
