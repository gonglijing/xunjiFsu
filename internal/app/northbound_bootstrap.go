...
func loadEnabledNorthboundConfigs(northboundMgr *northbound.NorthboundManager) {
	slog.Info("Loading enabled northbound configs...")
	...
	for _, config := range configs {
		...
		if err := registerNorthboundAdapter(northboundMgr, config, sysStatsCollector); err != nil {
			slog.Warn("Failed to register northbound config", "name", config.Name, "error", err)
			continue
		}
		slog.Info("Northbound config registered",
			"name", config.Name,
			"type", config.Type,
	...
	slog.Info("Loaded enabled northbound configs", "count", enabledCount)
}
...
func registerPandaxSystemStatsProvider(adapter adapters.NorthboundAdapter, config *models.NorthboundConfig, sysStatsCollector *collector.SystemStatsCollector) {
	...
	slog.Info("Set system stats provider for pandax adapter")
}
